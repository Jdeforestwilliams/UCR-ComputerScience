...
LD R6, input_arr
ADD R1, R1, #0
BRnp switch_array ;; used to switch to the second array when needed 
...
BRnp SKIP_ARRAY_SWITCH  ;; used to switch to the second array on the second run
LD R6, input_second_arr
SKIP_ARRAY_SWITCH
AND R2, R2, #0
ADD R6, R6, #1
...
AND R0, R0, #0  ;;; to clear all uneeded registers 
AND R3, R3, #0
AND R4, R4, #0
AND R5, R5, #0
AND R6, R6, #0
;;;;; to check for the sign of the product
;;;; R3 = -1 means R1 is negative
;;;; R0 =  1 means R2 is negative
;;;; R3 =  0 means both are positive
LD R5, neg_sign_1
LD R4, pos_sign_1
...
AND R3, R3, #0
sign_complete
;;; to restore registers back to 0
AND R4, R4, #0
AND R5, R5, #0
AND R6, R6, #0
;;; to check which one is bigger if both positive
ADD R3, R3, #0
BRzp both_positive  ;;; if both are positive which one is smaller
BRn reg_1_neg
both positive
NOT R1, R1
ADD R1, R1, #1
ADD R4, R1, R2
BRzp reg_1_sm_both_pos ;;; to branch if register 1 is smaller and both are positive
AND R3, R3, #0
ADD R5, R1, #0 ;; to store the bigger number in R5
reg_1_sm_both_pos 
AND R3, R3, #0
ADD R4, R2, #0
BRnzp END_ALL ;;; to skip to the end since we have bigger number
reg_1_neg
ADD R0, R0, #0
BRnz reg_2_pos ;;; to branch if register 2 is positive
NOT R1, R1
ADD R1, R1, #1
ADD R4, R1, R2
BRzp reg_1_bigger
ADD R5, R2, #0
BRnzp END_ALL ;;; to skip to end since we have bigger number
reg_1_bigger
ADD R5, R1, #0
BRnzp END_ALL
reg_2_pos
