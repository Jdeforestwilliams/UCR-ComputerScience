/*
 * adeja001_lab8_part2.c - April 8, 2013
 * Name: Ariana DeJaco  E-mail:adeja001@ucr.edu
 * CS Login: adeja001
 * Partner Name: Joshua DeForest-Williams jdefo002@ucr.edu
 * Lab Section: 022
 * Assignment: Lab#8 Exercise# Part2
 * Exercise Use the LCD code, along with a button and/or time delay to display the message "CS120B is Legend... wait for it DARY!" The string will not fit on the display all at once, 	*	so you will need to come up with some way to paginate or scroll the text.
*/
...
enum LI_States { Initial, LI_Init1, LI_Init2, LI_Init3, LI_Init4, LI_Init5, LI_Init6,
...
enum LT_States { Initial1, LT_s0, LT_WaitLcdRdy, LT_WaitButton, LT_FillAndDispStringPartOne, 
LT_FillAndDispStringPartTwo, LT_HoldGo1, LT_WaitBtnRelease } LT_State;

void LT_Tick() {
	//static unsigned char i, x, c;
	const char* sentencePartOne = "CS120B is Legend...";
	const char* sentencePartTwo = "wait for it DARY!"; 
	...
	switch(LT_State) { // Transitions
		...
		case LT_WaitButton:
			...
			else if (GetBit(PINA,0)==0 && ((previousState == 0) || (previousState == 2)) ) { // Button active low
				LT_State = LT_FillAndDispStringPartOne;
			}
			else if (GetBit(PINA,0)==0 && (previousState == 1) ) { // Button active low
			LT_State = LT_FillAndDispStringPartTwo;
			}			
			break;
		case LT_FillAndDispStringPartOne:
			LT_State = LT_HoldGo1;
			previousState = 1; 
			break;
		case LT_FillAndDispStringPartTwo: 
			LT_State = LT_HoldGo1;
			previousState = 2; 
			break; 
		case LT_HoldGo1:
	...
	switch(LT_State) { // State actions
		...
		case LT_WaitButton:
			break;
		case LT_FillAndDispStringPartOne:
			strcpy(LCD_string_g, sentencePartOne);
			LCD_string_g[strlen(sentencePartOne)] = '\0'; // End-of-string char
			LCD_go_g = 1; // Display string
			break;
		case LT_FillAndDispStringPartTwo:
			strcpy(LCD_string_g, sentencePartTwo);
			LCD_string_g[strlen(sentencePartTwo)] = '\0'; // End-of-string char
			LCD_go_g = 1; // Display string
			break;
		case LT_HoldGo1:
			break;
...
//	unsigned long int tmpGCD = findGCD(LCD_Tick, KeyPad_Tick);
	...
	unsigned long int GCD = LCD_Tick; //tmpGCD;
