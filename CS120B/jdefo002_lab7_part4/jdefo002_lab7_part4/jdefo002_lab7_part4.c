 ...
 *  EXENTSION 4: Extend the previous exercise to allow a user to adjust the sound frequency
 *  up or down using buttons connected to PA0 (up) and PA1 (down). Using our 1 ms 
 *  timer abstraction, the fastest you'll be able to pulse is 1 ms on and 1 ms off, 
 *  meaning 500 Hz. You'll probably want to introduce another synchSM that polls 
 *  the buttons and sets a global variable storing the current frequency that in 
 *  turn is read by the frequency generator task. 
